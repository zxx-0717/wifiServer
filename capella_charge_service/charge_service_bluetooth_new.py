...
class BluetoothServer(Node):
    def __init__(self, name):
        ...
        # 是否要开始连接蓝牙的变量
        self.bluetooth_connect = False
        # 是否断开蓝牙的属性
        self.disconnect_bluetooth = False
        # 蓝牙的mac地址
        self.charger_macid = "" # '94:C9:60:43:C0:6D'#充电桩
        # 蓝牙数据notify的uuid
        ...
        # 开启蓝牙线程
        self.thread_blue = threading.Thread(target=self.bluetooth_thread,daemon=True)
    ...
    def start_docking_charging_pile_visual_callback(self, request, response):
        ...
        print('重启蓝牙')
        os.system('bluetoothctl power off') # sudo rfkill block bluetooth
        time.sleep(2)
        os.system('bluetoothctl power on')# sudo rfkill unblock bluetooth
        time.sleep(2)
        self.charge_state.is_docking = True
        # 连接蓝牙
        self.bluetooth_connect = True
        self.charge_state.pid = ''
        t1 = time.time()
        # 等待蓝牙连接结果
    ...
    # 连接充电桩蓝牙      
    async def create_bleakclient(self,):
        while True:
            if self.bluetooth_connect:
                print('开始连接蓝牙')
                try:
                    # 开始连接蓝牙
                    self.charge_state.pid = ''
                    # print("开始创建bleakclient")
                    async with BleakClient(self.charger_macid) as self.bleak_client:
                        print('蓝牙已连接')
                        self.disconnect_bluetooth = False
                        # 获取蓝牙的服务
                        print('获取蓝牙服务中')
                        ...
                        for service in services:
                            ...
                            for character in service.characteristics:
                                ...
                                # 获取接收数据的蓝牙服务uuid
                                    ...
                                    continue
                            print('*************************************')
                        self.charge_state.pid = self.charger_macid
                        self.bluetooth_connect = False
                ...
                #     raise
                    ...
                    self.charge_state.pid = ""
                    print('----------create bleak报错：',e)
                    self.bluetooth_connect = False
                    continue
            else:
                self.charge_state.pid = ""
                # self.get_logger().info(f"等待连接蓝牙。")
                continue
        
    def bluetooth_thread(self,):
        asyncio.run(self.create_bleakclient())
...
def main(args=None):
    ...
    node = BluetoothServer('bluetooth_charge_server')
    rclpy.spin(node)
    rclpy.shutdown()
